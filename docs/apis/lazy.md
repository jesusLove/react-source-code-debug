# 文件路径 `packages/react/src/ReactLazy.js`
...
export function lazy<T>(
  ...
  const lazyType: LazyComponent<T, Payload<T>> = {
    $$typeof: REACT_LAZY_TYPE, // 定义在 `shared/ReactSymbols` 中
    _payload: payload,
...
// =>
{
  ...
  _init: lazyInitializer, // 初始化方法
}
```

主要看一下 `lazyInitialier` 方法中具体做了什么？`lazyInitialier` 中对这四种状态进行处理。源码如下：
